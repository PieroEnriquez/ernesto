...
function findExportedMetaLiteral(
    sf: ts.SourceFile,
    ...
    for (const stmt of sf.statements) {
        ...
        throw new DynamicWorkflowParseError(
            '`export const meta` must be initialized to an object literal',
            posLine(sf, decl.initializer),
        );
    }
    return undefined;
}
...
function parseLiteralExpression(node: ts.Expression): unknown {
    ...
    if (ts.isArrayLiteralExpression(node)) {
        return node.elements.map((el) => {
            if (ts.isSpreadElement(el) || el.kind === ts.SyntaxKind.OmittedExpression) {
                throw new DynamicWorkflowParseError(
                    'spread / omitted elements are not allowed in `meta`',
                );
            }
    ...
    if (ts.isObjectLiteralExpression(node)) {
        ...
        for (const prop of node.properties) {
            if (!ts.isPropertyAssignment(prop)) {
                throw new DynamicWorkflowParseError(
                    'shorthand / spread / method properties are not allowed in `meta`',
                );
            }
    ...
    throw new DynamicWorkflowParseError(
        `non-literal expression in \`meta\` (kind=${ts.SyntaxKind[node.kind]})`,
    );
}
...
function readPropertyName(name: ts.PropertyName): string {
    ...
    throw new DynamicWorkflowParseError(
        'computed property names are not allowed in `meta`',
    );
}
...
function validateMetaShape(
    ...
    if (meta.whenToUse !== undefined && typeof meta.whenToUse !== 'string') {
        throw new DynamicWorkflowParseError(
            '`meta.whenToUse` must be a string when present',
        );
    }
    if (meta.phases !== undefined) {
        if (!Array.isArray(meta.phases)) {
            throw new DynamicWorkflowParseError(
                '`meta.phases` must be an array when present',
            );
        }
        for (const [i, phase] of meta.phases.entries()) {
            if (!isPlainObject(phase)) {
                throw new DynamicWorkflowParseError(
                    `\`meta.phases[${i}]\` must be an object`,
                );
            }
            if (typeof phase.title !== 'string') {
                throw new DynamicWorkflowParseError(
                    `\`meta.phases[${i}].title\` must be a string`,
                );
            }
            if (typeof phase.detail !== 'string') {
                throw new DynamicWorkflowParseError(
                    `\`meta.phases[${i}].detail\` must be a string`,
                );
            }
        }
    }
    if (meta.model !== undefined && typeof meta.model !== 'string') {
        throw new DynamicWorkflowParseError(
            '`meta.model` must be a string when present',
        );
    }
    if (
