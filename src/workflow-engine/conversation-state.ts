...
import type { HitlComponent, HitlExpect, RenderableComponent } from '../components/types';
...
export type ConversationStatus = 'new' | 'running' | 'awaiting_input' | 'completed' | 'errored' | 'canceled';
 ...
 */
export async function loadConversationState(workdir: string): Promise<ConversationState | undefined> {
 ...
 */
export async function saveConversationState(workdir: string, state: ConversationState): Promise<void> {
    const filePath = stateFilePath(workdir);
...
export async function updateConversationState(
    ...
    const prev = (await loadConversationState(workdir)) ?? {
 ...
 */
export async function appendHitlToTrail(workdir: string, runId: string, hitl: HitlComponent): Promise<void> {
    ...
    await updateConversationState(workdir, (prev) => {
        ...
        const capped = trail.length > UI_TRAIL_CAP ? trail.slice(trail.length - UI_TRAIL_CAP) : trail;
...
export interface RendererPromptStrategy {
    ...
    forCompleted?(input: RendererInput & { kind: 'new_message' }, prev: RendererStrategyPrev): string;
    /** Human interrupted a running agent turn with a new message. */
    forRunningPreempted?(input: RendererInput & { kind: 'new_message' }, prev: RendererStrategyPrev): string;
    ...
    forAwaitingInputResolved?(input: RendererInput & { kind: 'hitl_response' }, pending: PendingHitlRecord): string;
    ...
    forCanceled?(input: RendererInput & { kind: 'new_message' }, prev: RendererStrategyPrev): string;
}
...
export const defaultRendererStrategy: Required<RendererPromptStrategy> = {
    ...
    forRunningPreempted: (input, _prev) => `[The user interrupted your previous response with a new message:]\n\n${input.text}`,
    ...
    forErrored: (input, error, _prev) =>
        `[Your previous turn errored${error ? `: ${error.message}` : ''}. The user now writes:]\n\n${input.text}`,
    forCanceled: (input, _prev) => `[Your previous turn was canceled. The user now writes:]\n\n${input.text}`,
};
 ...
 */
export function composeStrategy(custom: RendererPromptStrategy | undefined): Required<RendererPromptStrategy> {
    ...
    return {
        ...
        forRunningPreempted: custom.forRunningPreempted ?? defaultRendererStrategy.forRunningPreempted,
        forAwaitingInputResolved: custom.forAwaitingInputResolved ?? defaultRendererStrategy.forAwaitingInputResolved,
        forAwaitingInputPreempted: custom.forAwaitingInputPreempted ?? defaultRendererStrategy.forAwaitingInputPreempted,
...
export function decideRendererAction(
    ...
    const prev: RendererStrategyPrev = state?.uiTrail ? { uiTrail: state.uiTrail } : {};

    // ── HITL response — must match the pending pause's promptId ───
    if (input.kind === 'hitl_response') {
        if (state?.status === 'awaiting_input' && state.pendingHitl && state.pendingHitl.promptId === input.promptId) {
            return {
                kind: 'dispatch_continuation',
